...
export function VoiceNote({ name, initials, duration }) {
  const bars = [6, 12, 18, 10, 22, 14, 9, 20, 12, 17, 8, 15, 11, 19, 9, 13, 7, 16, 10, 14];
  return (
    <div
      style={{
        background: "#FFFFFF",
    ...
    >
      <span style={{ color: C.success, fontWeight: 700, fontSize: 13, fontFamily: "'Inter', sans-serif" }}>{name}</span>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 2, flex: 1, height: 24 }}>
          {bars.map((h, i) => (
            <div
              key={i}
              style={{
                ...
                height: h,
                borderRadius: 2,
                background: i < 5 ? C.primary : C.panelBorder,
              }}
            />
          ))}
        </div>
      ...
      <span style={{ color: C.mutedDark, fontSize: 11.5 }}>{duration}</span>
    </div>
  );
}
